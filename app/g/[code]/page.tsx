...
import { authorsOf, isAuthor } from "@/app/lib/authors";
...
export default function StudentRoom({params}:{params:Promise<{code:string}>}) {
  ...
  // Работа может быть командной: показываем её и соавторам, не только загрузившему.
  const myPresentations=useMemo(()=>data?.presentations.filter(p=>isAuthor(p,name))||[],[data,name]);
  ...
  async function upload(e:FormEvent<HTMLFormElement>){
    ...
    try{
      const init=await fetch(`/api/groups/${code}/uploads`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({studentName:name,authors:String(fd.get("authors")||name),title,filename:file.name,size:file.size,type:file.type})});
  ...
  return <main className="student-page">
    ...
    <section className="student-card upload-card">
      ...
      {myPresentations.length>0&&<div className="my-uploads"><h2>Загруженные работы <span>{myPresentations.length}</span></h2>{myPresentations.map(p=><div className="my-upload-row" key={p.id}><span className="pdf-badge">PDF</span><div><b>{p.title}</b><small>{authorsOf(p)} · {p.filename}</small></div><strong>Загружено ✓</strong></div>)}</div>}
      ...
      <form onSubmit={upload} aria-busy={busy}>
        ...
        <label>Авторы работы<input name="authors" defaultValue={name} disabled={busy} placeholder="Иванов Иван, Петров Пётр"/><small className="field-hint">Если работу защищает команда, перечислите всех через запятую</small></label>
...
function StudentPresentation({active,page}:{active:Presentation;page:number}){
  ...
  return <main className="student-stage" ref={stageRef}>
    <header><span className="live"><i/> ПРЯМОЙ ЭФИР</span><div><b>{active.title}</b><small>{authorsOf(active)}</small></div><div className="student-stage-actions"><span className="follow">Слайд {page}{total>0?` из ${total}`:""}</span><button className="ghost-button" onClick={()=>void toggleFullscreen(stageRef.current)}>{fullscreen?"⤡ Выйти":"⛶ Полный экран"}</button></div></header>
